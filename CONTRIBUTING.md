...
Edit canonical content in repository-root files and source directories:

- Top-level docs (`README.md`, `GETTING-STARTED.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, `references.md`)
- `learn/`
- `prompts/`

The `docs_src/` tree is generated via symlinks for MkDocs and is not a separate content source. Use `make sync` to refresh links when needed.
...
This section walks you through creating a new prompt template from scratch.
...
We use `make` for common development tasks. Run `make help` to see all targets.

| Command | Description |
|---------|-------------|
| `make install` | Install / refresh docs + dev dependencies |
| `make sync` | Create `docs_src/` symlinks to `learn/`, `prompts/`, and top-level docs |
| `make sync-check` | Verify `docs_src/` symlinks resolve correctly |
| `make recursive-symlink-check` | Ensure no recursive/self-referential symlinks exist in tracked content paths |
| `make build` | Sync `docs_src/`, then build the MkDocs static site into `_site/` |
| `make serve` | Sync `docs_src/`, then start the MkDocs dev server (live-reload) |
| `make lint` | Run all prompt-file linters (frontmatter + cross-link + required sections) |
| `make validate` | Run the full YAML schema validator for prompt files |
| `make check` | Run full checks including docs build |
