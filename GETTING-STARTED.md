...
Use this path if you want a working setup first, then details.

1. Download and run the setup script for your stack (example: Python stack) in your project root using a pinned commit and checksum verification:

```bash
SCRIPT_COMMIT=ba24422
curl -fsSL -o setup.sh "https://raw.githubusercontent.com/kunalsuri/prompt-engineering-playbook/${SCRIPT_COMMIT}/scripts/setup.sh"
echo "71731388b982ae9076ec8b3963f2986eb5f34ca3eb27e1607ca93e97ccc0832b  setup.sh" | shasum -a 256 -c -
bash setup.sh --stack python "$PWD"
rm setup.sh
```

2. Confirm these files now exist:
...
3. Open Copilot Chat and run a prompt command (for example, `/create-feature`).

After this quick start, continue with the full guide below for stack selection, customization, and troubleshooting.

---

## Manual Install (No `curl | bash`)

Use this path if your environment disallows running remote scripts.
...
2. In your target project root, create destination directories:

```bash
mkdir -p .github/prompts
```

3. Copy stack files from the cloned repository (example: Python stack):
...
4. Open VS Code in your target project and continue with [Step 5: Use a Prompt File](#step-5-use-a-prompt-file).

---

## Post-Install Verification Checklist

Use this checklist after either the fast-path script or manual install:

- [ ] `.github/copilot-instructions.md` exists in your project root.
- [ ] `.github/prompts/` exists and contains one or more `.prompt.md` files.
- [ ] Copilot Chat `/` autocomplete shows your installed prompt names.
- [ ] Running one prompt command (for example `/create-feature`) starts an agent flow.
...
- **VS Code** (version 1.96 or later) with the [GitHub Copilot extension](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot) installed and activated with a Copilot Pro, Business, or Enterprise subscription.
- **Git** installed and configured on your system.
- A target project where you want to use the prompt templates. This can be an existing repository or a new one.

---

## Sandbox Requirement (Strongly Enforced)

To prevent failures and accidental system-level package changes, run repository scripts in an isolated Python virtual environment (`.venv`) only.

### Required setup for local repository scripts
...
Several repo scripts now enforce this policy and will stop with a clear error if executed outside a virtual environment.
...
### Option B: Use the Setup Scripts (Recommended)

If you have an existing project, the automated setup scripts in the `scripts/` directory will fetch and install the right templates for you. The following example demonstrates the process for a Python project.

```bash
# Navigate to your project root
cd /path/to/your-project

# Download and run the setup script for your stack (pinned commit + checksum)
SCRIPT_COMMIT=ba24422
curl -fsSL -o setup.sh "https://raw.githubusercontent.com/kunalsuri/prompt-engineering-playbook/${SCRIPT_COMMIT}/scripts/setup.sh"
echo "71731388b982ae9076ec8b3963f2986eb5f34ca3eb27e1607ca93e97ccc0832b  setup.sh" | shasum -a 256 -c -
bash setup.sh --stack python "$PWD"
...
*(Change `python` to `react-typescript`, `react-fastapi`, or `nodejs-typescript` based on your stack. Run `bash setup.sh --list` to see all options, or `bash setup.sh --dry-run --stack <stack>` to preview without copying.)*
