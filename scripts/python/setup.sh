#!/usr/bin/env bash
# setup.sh — Set up the Python prompt templates in a target project.
#
# Usage:
#   ./setup.sh /path/to/your-project
#
# This script copies the Python copilot-instructions and prompt files
# into the target project's .github/ directory, following the VS Code
# Copilot prompt-file convention.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
PYTHON_DIR="$REPO_ROOT/prompts/python"
...
if [[ $# -lt 1 ]]; then
  ...
  echo ""
  echo "Copies Python prompt templates into your project's .github/ directory."
  echo ""
  echo "Example:"
  echo "  $0 ~/projects/my-python-app"
  exit 1
fi
...
echo "Setting up prompt templates in: $TARGET_DIR"
...
cp "$PYTHON_DIR/copilot-instructions.md" "$TARGET_DIR/.github/copilot-instructions.md"

# --- Copy prompt files ---
echo "  Copying prompt files ..."
PROMPT_COUNT=0
for prompt_file in "$PYTHON_DIR/prompts/"*.prompt.md; do
