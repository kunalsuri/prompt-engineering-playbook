...
Usage:
    .venv/bin/python scripts/validate-prompt-schema.py [file ...]

    # Validate all prompt files:
    .venv/bin/python scripts/validate-prompt-schema.py prompts/**/*.prompt.md

    # Validate a single file:
    .venv/bin/python scripts/validate-prompt-schema.py prompts/python/prompts/write-tests.prompt.md
...
def ensure_virtual_environment() -> None:
    """Fail fast when script is executed outside a virtual environment."""
    virtual_env = os.environ.get("VIRTUAL_ENV")
    if virtual_env:
        return

    # Fallback detection for venv/virtualenv where VIRTUAL_ENV may be unset.
    if getattr(sys, "base_prefix", sys.prefix) != sys.prefix:
        return

    print("ERROR: This script must run inside an isolated virtual environment (.venv).")
    print("Run:")
    print("  python3 -m venv .venv")
    print("  source .venv/bin/activate")
    print("  python -m pip install -r requirements-dev.txt")
    print("Then retry with:")
    print("  .venv/bin/python scripts/validate-prompt-schema.py")
    sys.exit(2)
...
def load_runtime_dependencies() -> None:
    """Import third-party dependencies after sandbox checks pass."""
    global yaml, Draft202012Validator

    try:
        import yaml as _yaml
        from jsonschema import Draft202012Validator as _Draft202012Validator
    except ImportError:
        print("ERROR: Missing dependencies in current virtual environment. Install with:")
        print("  python -m pip install -r requirements-dev.txt")
        sys.exit(2)
...
def extract_frontmatter(path: Path) -> dict | None:
    ...
    if end_index == -1:
        print("  YAML parse error: frontmatter closing delimiter '---' not found")
        return None
