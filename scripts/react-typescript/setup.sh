#!/usr/bin/env bash
# setup.sh — Set up the React + TypeScript prompt templates in a target project.
#
# Usage:
#   ./setup.sh /path/to/your-project
#
# This script copies the React + TypeScript copilot-instructions and prompt
...
REACT_TS_DIR="$REPO_ROOT/prompts/react-typescript"
...
if [[ $# -lt 1 ]]; then
  ...
  echo ""
  echo "Copies React + TypeScript prompt templates into your project's .github/ directory."
  echo ""
  echo "Example:"
  echo "  $0 ~/projects/my-react-app"
  exit 1
fi
...
echo "Setting up React + TypeScript prompt templates in: $TARGET_DIR"
...
cp "$REACT_TS_DIR/copilot-instructions.md" "$TARGET_DIR/.github/copilot-instructions.md"

# --- Copy prompt files ---
echo "  Copying prompt files ..."
PROMPT_COUNT=0
for prompt_file in "$REACT_TS_DIR/prompts/"*.prompt.md; do
