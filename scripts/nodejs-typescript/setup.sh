#!/usr/bin/env bash
# setup.sh — Set up the Node.js + TypeScript prompt templates in a target project.
#
# Usage:
#   ./setup.sh /path/to/your-project
#
# This script copies the Node.js + TypeScript copilot-instructions and prompt
...
NODEJS_TS_DIR="$REPO_ROOT/prompts/nodejs-typescript"
...
if [[ $# -lt 1 ]]; then
  ...
  echo ""
  echo "Copies Node.js + TypeScript prompt templates into your project's .github/ directory."
  echo ""
  echo "Example:"
  echo "  $0 ~/projects/my-node-api"
  exit 1
fi
...
echo "Setting up Node.js + TypeScript prompt templates in: $TARGET_DIR"
...
cp "$NODEJS_TS_DIR/copilot-instructions.md" "$TARGET_DIR/.github/copilot-instructions.md"

# --- Copy prompt files ---
echo "  Copying prompt files ..."
PROMPT_COUNT=0
for prompt_file in "$NODEJS_TS_DIR/prompts/"*.prompt.md; do
