...
from lab_utils import complete, ensure_sandbox_environment, get_client, print_comparison_table, print_header, resolve_model
    ...
    for _ in range(RUNS_PER_VARIANT):
        try:
            ...
            if not response.choices:
                results.append({"success": False, "error": "No completion choices returned", "raw": ""})
                continue
            ...
            results.append({"success": False, "error": f"{type(exc).__name__}: {exc}", "raw": ""})
    return results
