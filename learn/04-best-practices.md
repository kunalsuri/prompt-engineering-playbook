...
**Single source of truth.** Shared instructions (e.g., code style rules, documentation standards, common linting configurations) should exist in exactly one file, referenced by other prompts rather than duplicated. If your Python style guide appears in three places, it will inevitably diverge. This repository follows this principle by placing shared resources (evaluation template, README-generator base) in `prompts/shared/` and referencing them via `#file:` directives.
